...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    //    }
    NSString *htmlHeadString = @"ulitewebview://";
    if([urlStr hasPrefix:htmlHeadString]){
        NSLog(@"拒绝请求：%@",urlStr);
        NSString *targetString = [urlStr componentsSeparatedByString:htmlHeadString][1];

        UnitySendMessage([_gameObjectName UTF8String], "OnJsCall", [[targetString stringByRemovingPercentEncoding] UTF8String]);
        decisionHandler(WKNavigationActionPolicyCancel);
    }else{
        NSLog(@"同意请求：%@",urlStr);
        decisionHandler(WKNavigationActionPolicyAllow);
